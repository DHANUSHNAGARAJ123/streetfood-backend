...
public interface VendorRepository extends JpaRepository<Vendor, Long> {
    ...
    @Query("SELECT v FROM Vendor v WHERE v.status = 'APPROVED' AND v.lat IS NOT NULL AND v.lng IS NOT NULL AND " +
           "(6371 * acos(cos(radians(:lat)) * cos(radians(v.lat)) * " +
           "cos(radians(v.lng) - radians(:lng)) + sin(radians(:lat)) * sin(radians(v.lat)))) < :radius")
